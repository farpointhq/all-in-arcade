...
Modes (default all):
  live    — boots the REAL app at ?level=amir-kermany-nano-cure-2&bot=1&flavor=good
            ...
            errors; hearts + trace recorded per run; a loss saves a full-res
            shot-loss-run<N>.png + final state as triage evidence. Known booth-wide
            logo-404 noise is filtered.
...
def run_live_once(idx):
    ...
    out = {"run": idx, "console_errors": [], "page_error": None, "won": False,
    ...
    with sync_playwright() as pw:
        ...
        def on_console(m):
            if m.type != "error":
                return
            txt = m.text or ""
            if "logo-all.svg" in txt or "logo-in.svg" in txt or "Failed to load resource" in txt or "404" in txt:
                return  # booth-wide masthead 404, not this lane (maze-genre note)
            out["console_errors"].append(txt)
    ...
    print("[live run %d] won=%s hearts=%s page_error=%s console_errors=%s trace=%s" % (
        idx, out["won"], out["hearts"], out["page_error"], out["console_errors"], json.dumps(out["trace"])))
    if not out["won"]:
