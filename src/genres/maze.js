// MAZE genre — old-school Pac-style maze chaser, reimagined as a school hallway.
// Built for "Study Hall Scramble" (Kristopher Federico's level): munch every
// chalk fact, dodge the staff — Hall Monitor (red, direct), Math Teacher (pink,
// ambush), Librarian (cyan, flanks), Janitor (orange, shy) — gold apples grant
// a RECESS power window where the staff turns pale and can be chased for credit.
//
// Arcade-canon mechanics: tile-tween movement, buffered turns + instant
// reversal, wrap-around tunnel with slow-down, scatter/chase waves with
// per-ghost AI targets, frightened blinking, eaten eyes fly home to the lounge,
// courage ramp (the hall monitor speeds up when few facts remain).
//
...
const DEF = {
  player: 114, ghost: 102, fright: 64, eyes: 205,
  frightSeconds: 7, tunnelSlow: 0.55,
  release: [0, 2.4, 4.8, 7.2],       // staff leave the lounge, staggered
  elroy: [[26, 1.07], [12, 1.18]],   // hall-monitor rage steps (few facts left)
};
...
function paintGhost(g, w, h, spec, frame, fear) {
  ...
  const dark = fear ? "#5fb9c4" : spec.dark;
...
// ---- dungeon personas (Knight's Dungeon Scramble remix — Louis-Philippe Gaulin) ----
// Same four AI brains (idx order = direct/ambush/flank/shy), medieval bodies.
const PERSONAS_DUNGEON = [
  { key: "goblin", body: "#7fae3f", dark: "#3f5c1c", look: 1, // Goblin: pointed ears + dagger
    paint: (g, cx, h) => {
      g.fillStyle = "#5c8c2c"; g.strokeStyle = INK; g.lineWidth = 2;
      g.beginPath(); g.moveTo(cx - 15, h * 0.42); g.lineTo(cx - 24, h * 0.30); g.lineTo(cx - 14, h * 0.36); g.closePath(); g.fill(); g.stroke();
      g.beginPath(); g.moveTo(cx + 15, h * 0.42); g.lineTo(cx + 24, h * 0.30); g.lineTo(cx + 14, h * 0.36); g.closePath(); g.fill(); g.stroke();
      g.save(); g.translate(cx + 13, h - 20); g.rotate(0.6);
      g.strokeStyle = "#c9b48a"; g.lineWidth = 3; g.beginPath(); g.moveTo(0, 0); g.lineTo(0, -13); g.stroke();
      g.fillStyle = "#8a6a3a"; g.beginPath(); g.arc(0, 2, 3.4, 0, PI2); g.fill();
      g.restore();
    } },
  { key: "wraith", body: "#c98fd9", dark: "#6f3f85", look: 1, // Wraith: hood + tattered hem glow
    paint: (g, cx, h) => {
      g.fillStyle = "#4a2c5c"; g.strokeStyle = INK; g.lineWidth = 2;
      g.beginPath(); g.moveTo(cx - 12, h * 0.40); g.quadraticCurveTo(cx, h * 0.10, cx + 12, h * 0.40);
      g.quadraticCurveTo(cx, h * 0.30, cx - 12, h * 0.40); g.closePath(); g.fill(); g.stroke();
      g.strokeStyle = "rgba(214,160,255,.8)"; g.lineWidth = 2;
      g.beginPath();
      for (let i = 0; i < 4; i++) { g.moveTo(cx - 12 + i * 8, h - 8); g.lineTo(cx - 12 + i * 8 + 3, h - 3); }
      g.stroke();
    } },
  { key: "bat", body: "#5fc8e8", dark: "#226a8c", look: -1, // Bat: wings + fangs
    paint: (g, cx, h) => {
      g.fillStyle = "#3d84a8"; g.strokeStyle = INK; g.lineWidth = 2;
      for (const s of [-1, 1]) {
        g.beginPath(); g.moveTo(cx + s * 6, h * 0.44);
        g.quadraticCurveTo(cx + s * 20, h * 0.24, cx + s * 24, h * 0.52);
        g.quadraticCurveTo(cx + s * 17, h * 0.44, cx + s * 12, h * 0.62);
        g.quadraticCurveTo(cx + s * 8, h * 0.55, cx + s * 6, h * 0.52);
        g.closePath(); g.fill(); g.stroke();
      }
      g.fillStyle = "#fff"; g.beginPath(); g.moveTo(cx - 3, h * 0.66); g.lineTo(cx - 1, h * 0.74); g.lineTo(cx + 1, h * 0.66); g.closePath(); g.fill();
      g.beginPath(); g.moveTo(cx + 2, h * 0.66); g.lineTo(cx + 4, h * 0.74); g.lineTo(cx + 6, h * 0.66); g.closePath(); g.fill();
    } },
  { key: "orc", body: "#f5a23f", dark: "#a3621b", look: 1, // Orc: tusks + horned helm
    paint: (g, cx, h) => {
      g.fillStyle = "#8a6a3a"; g.strokeStyle = INK; g.lineWidth = 2;
      g.beginPath(); g.moveTo(cx - 14, h * 0.34); g.lineTo(cx - 17, h * 0.12); g.lineTo(cx - 9, h * 0.26); g.closePath(); g.fill(); g.stroke();
      g.beginPath(); g.moveTo(cx + 14, h * 0.34); g.lineTo(cx + 17, h * 0.12); g.lineTo(cx + 9, h * 0.26); g.closePath(); g.fill(); g.stroke();
      g.fillStyle = "#fff"; g.strokeStyle = INK; g.lineWidth = 1.4;
      g.beginPath(); g.moveTo(cx - 6, h * 0.70); g.lineTo(cx - 4, h * 0.80); g.lineTo(cx - 2, h * 0.70); g.closePath(); g.fill(); g.stroke();
      g.beginPath(); g.moveTo(cx + 2, h * 0.70); g.lineTo(cx + 4, h * 0.80); g.lineTo(cx + 6, h * 0.70); g.closePath(); g.fill(); g.stroke();
    } },
];

// WYRM — the goblin's elite re-spawn: fire crown, hotter colors (RPG boss-lite)
const PERSONA_WYRM = { key: "wyrm", body: "#e8452c", dark: "#7c1a12", look: 1,
  paint: (g, cx, h) => {
    g.strokeStyle = "#ffb03f"; g.lineWidth = 3.4; g.lineCap = "round";
    g.beginPath();
    for (let i = -2; i <= 2; i++) {
      g.moveTo(cx + i * 5, h * 0.30);
      g.quadraticCurveTo(cx + i * 5 + 3, h * 0.14, cx + i * 5 - 2, h * 0.06);
    }
    g.stroke();
    g.fillStyle = "#ffd23f";
    g.beginPath(); g.arc(cx - 8, h * 0.42, 2.6, 0, PI2); g.fill();
    g.beginPath(); g.arc(cx + 8, h * 0.42, 2.6, 0, PI2); g.fill();
  } };

const GF = 72;
function bakeGhosts(specs) {
  return specs.map((p) =>
    [0, 1].map((f) => {
...
const RUNE = (() => {
  ...
  g.fillStyle = "#4f6a60"; g.strokeStyle = "#223730"; g.lineWidth = 2.4;
  rr(g, 12, 10, 20, 24, 4); g.fill(); g.stroke();
  g.fillStyle = "#7ee6d2"; g.strokeStyle = "#1c443e"; g.lineWidth = 1.8;
  g.beginPath();
  g.moveTo(22, 15); g.lineTo(29, 20); g.lineTo(29, 27); g.lineTo(22, 31); g.lineTo(15, 27); g.lineTo(15, 20);
  g.closePath(); g.fill(); g.stroke();
  g.strokeStyle = "#133a33"; g.lineWidth = 1.6;
  g.beginPath(); g.moveTo(18, 23); g.lineTo(22, 19); g.lineTo(26, 23); g.lineTo(22, 27); g.lineTo(18, 23); g.stroke();
  g.fillStyle = "#c78a3a"; g.strokeStyle = "#6e4b1a"; g.lineWidth = 1.6;
  g.beginPath(); g.arc(22, 35.5, 2.4, 0, PI2); g.fill(); g.stroke();
  return c;
})();
...
const SCROLL = (() => {
  const c = document.createElement("canvas"); c.width = 52; c.height = 52;
  const g = c.getContext("2d");
  g.fillStyle = "#e8d9a8"; g.strokeStyle = "#6e5920"; g.lineWidth = 2;
  rr(g, 14, 20, 24, 14, 3); g.fill(); g.stroke();
  g.fillStyle = "#d9c489"; g.strokeStyle = "#6e5920";
  rr(g, 10, 16, 7, 22, 3); g.fill(); g.stroke();
  rr(g, 35, 16, 7, 20, 3); g.fill(); g.stroke();
  g.strokeStyle = "#6e5920"; g.lineWidth = 1.4;
  g.beginPath(); g.moveTo(19, 24); g.lineTo(31, 24); g.moveTo(19, 28); g.lineTo(31, 28); g.stroke();
  g.fillStyle = "#c23a2a"; g.strokeStyle = "#6e1b14";
  g.beginPath(); g.arc(26, 37, 3.6, 0, PI2); g.fill(); g.stroke();
  return c;
})();
...
export function create(level, api) {
  ...
  // ---- theme + RPG config (School build stays at defaults) ----------------------
  const DUNGEON = st0.hero === "knight";
  const PERSONAS = DUNGEON ? PERSONAS_DUNGEON : PERSONAS_SCHOOL;
  const R = (data.rpg && data.rpg.enabled) ? Object.assign({
    titles: ["Squire", "Knight", "Champion", "Legend"], thresholds: [0, 1000, 2500, 4500],
    speedPerLevel: 1.06, speedCap: 1.25, shieldsPerLevel: 1, shieldMax: 2,
    coinXP: [10, 25, 50], runeXP: 200, slayXP: 400, questBonusXP: 1000,
    wyrmSlayThreshold: 2, wyrmSpeedMult: 1.16, wyrmFrightSeconds: 2, invulnSeconds: 1.2,
  }, data.rpg) : null;
  const names = data.names || {};
  const ROLE_NAMES = (data.names && data.names.roles) || [];
  const TERROR_WORD = names.terrorLabel || (DUNGEON ? "TERROR!" : "RECESS!");
  const PEN_LABEL = names.pen || (DUNGEON ? "THE KEEP" : "STAFF");
  const torches = [];

  // baked sprite sets for this level's persona array
  const GHOST_SPRITES = bakeGhosts(PERSONAS);
  const WYRM_SPRITES = bakeGhosts([PERSONA_WYRM]);
  ...
  const dots = {};
  let totalFacts = 0, coinSeq = 0;
  for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
    const ch = kind[c + "," + r];
    if (ch === "o") { dots[c + "," + r] = { v: 50, apple: true, got: false }; totalFacts++; }
    else if (ch === ".") {
      let tier = 0, v = 10;
      if (R) { coinSeq++; tier = coinSeq % 50 === 0 ? 2 : coinSeq % 20 === 0 ? 1 : 0; v = R.coinXP[tier]; }
      dots[c + "," + r] = { v, apple: false, tier, got: false }; totalFacts++;
    }
  }
  ...
  function buildGhosts(relMul) {
    ghosts = PERSONAS.map((p, i) => ({
      ...
      phase: i === 0 ? "roam" : "pen",
      wait: i === 0 ? 0 : tune.release[i] * relMul,
      fright: false, eaten: false, rev: false, bob: i * 0.9, wyrm: false, frightUntil: null,
    }));
  }
  ...
  const hudMid = () =>
    `${DUNGEON ? "GLORY" : "SCORE"} ${String(G.score).padStart(5, "0")} · FACTS ${G.facts}/${totalFacts}` +
    (R ? ` · XP ${String(R.titles[G.xpLvl] || "SQUIRE").toUpperCase()} ${G.xp}/${R.thresholds[Math.min(G.xpLvl + 1, R.thresholds.length - 1)]}` + (G.shields > 0 ? ` · SHIELD x${G.shields}` : "") : "") +
    (G.fright > 0 ? ` · ${TERROR_WORD.replace("!", "")} ${Math.ceil(G.fright)}s` : "");
  function freshState() {
    G = {
      status: "ready", tReady: 1.7, t: 0,
      score: 0, lives: (DBG || AP) ? Math.max(3, parseInt(Q.get("lives") || "3", 10) || 3) : 3, facts: totalFacts,
      wave: 0, waveT: 0, fright: 0, chain: 0,
      applesEaten: 0, notebook: null,
      popups: [], parts: [],
      dying: 0, bell: 0,
      xp: 0, xpLvl: 0, shields: 0, invuln: 0,
      goblinSlain: 0, wyrmSlain: false, wyrmAnnounced: false,
    };
    P = mkPlayer();
    buildGhosts(1);
  ...
  function eatAt(c, r) {
    ...
    if (d.apple) {
      ...
      for (const g of ghosts) if (!g.eaten && (g.phase === "roam" || g.phase === "exit")) { g.fright = true; g.rev = true; g.frightUntil = G.t + (g.wyrm && R ? R.wyrmFrightSeconds : tune.frightSeconds); }
      popup(ax, ay, TERROR_WORD, DUNGEON ? "#7de2c8" : "#a8ecf2", 17);
      burst(ax, ay, DUNGEON ? "#7de2c8" : "#a8ecf2", 12, 90);
      api.audio.sfx("jump");
  ...
  function addXp(n) {
    if (!R || n <= 0) return;
    G.xp += n;
    while (G.xpLvl + 1 < R.thresholds.length && G.xp >= R.thresholds[G.xpLvl + 1]) {
      G.xpLvl++;
      if (G.shields < R.shieldMax) G.shields += R.shieldsPerLevel;
      const a = anchor(P);
      const title = String(R.titles[G.xpLvl] || "Hero").toUpperCase();
      popup(a.x, a.y - 14, "LEVEL UP — " + title + "!", GOLD, 16);
      burst(a.x, a.y, GOLD, 16, 110);
      api.audio.sfx("win");
    }
  }

  // ---- ghost brains ----
  const WAVES = [["scatter", 4.2], ["chase", 15], ["scatter", 4.2], ["chase", 18], ["scatter", 3.6], ["chase", 9999]];
  ...
  function ghostTarget(g) {
    ...
    switch (g.idx) {
      ...
      case 1: return chase ? { c: pc.c + pd.x * 4, r: pc.r + pd.y * 4 } : SCATTER[1]; // Math Teacher: ambush 4 ahead
  ...
  function buildEyesScript(g) {
    ...
    g.done = () => {
      g.phase = "pen"; g.eaten = false; g.fright = false; g.dir = null; g.wait = 0.8; g.frightUntil = null;
      if (R && !g.wyrm && g.idx === 0) {
        G.goblinSlain++;
        if (G.goblinSlain >= R.wyrmSlayThreshold) g.wyrm = true;
      }
    };
  }
  function exitScript(g) {
    ...
    g.done = () => {
      g.phase = "roam"; g.dir = null;
      if (g.wyrm && !G.wyrmAnnounced) {
        G.wyrmAnnounced = true;
        const a0 = anchor(g);
        popup(a0.x, a0.y - 16, "THE WYRM RISES!", "#ff9c3f", 20);
  ...
  function stepGhost(g, dt) {
    ...
    if (g.fright && g.frightUntil != null && G.t >= g.frightUntil) g.fright = false;
    if (g.phase === "pen") {
      g.wait -= dt;
  ...
  function stepPlayer(dt) {
    const sp = tune.player * (G.fright > 0 ? 1.06 : 1) * (R ? Math.min(R.speedCap, Math.pow(R.speedPerLevel, G.xpLvl)) : 1);
    if (P.tx == null) {
  ...
  function afterDeath() {
    ...
    if (G.lives <= 0) {
      G.status = "lost";
      api.audio.sfx("lose");
      api.fail(DUNGEON ? "Fell in the dungeon — the monsters got you!" : "Sent to detention — the staff caught you!");
      recordRun(false);
    } else {
      P = mkPlayer();
      buildGhosts(0.55);
  ...
  function eatGhost(g) {
    ...
    if (R) addXp(R.slayXP);
    const a = anchor(g);
    const foe = g.wyrm ? "WYRM" : String(ROLE_NAMES[g.idx] || g.spec.key).toUpperCase();
    popup(a.x, a.y - 8, foe + " +" + val, ["#4dffb8", "#7dfcff", "#ffd23f", "#ff7ab8"][Math.min(G.chain - 1, 3)], R ? 13 : 15);
    if (g.wyrm && !G.wyrmSlain) {
      G.wyrmSlain = true;
      G.score += 1000;
      addXp(R ? R.questBonusXP : 0);
      popup(a.x, a.y - 32, "WYRM SLAIN! +1000 GLORY", "#ff9c3f", 17);
    }
  ...
  function botStep(dt) {
    ...
    for (const g of threats) {
      ...
      for (let dx = -shadow; dx <= shadow; dx++) for (let dy = -shadow; dy <= shadow; dy++) {
        if (Math.abs(dx) + Math.abs(dy) <= shadow) blocked.add(wrapC(g.fx + dx) + "," + clamp(g.fy + dy, 0, ROWS - 1));
      }
    }
    // don't route into short dead-end corridors while a threat is near (corner-trap fix)
    for (let rr2 = 0; rr2 < ROWS; rr2++) for (let cc = 0; cc < COLS; cc++) {
  ...
  (function bakeBG() {
    ...
    if (!DUNGEON) {
      ...
      // dungeon keep: hanging heraldic crests in the letterbox
      for (const s of [{ x0: 0, x1: ox - 8 }, { x0: ox + BW + 8, x1: W }]) {
        g.fillStyle = "rgba(6,8,14,.66)"; g.fillRect(s.x0, 0, s.x1 - s.x0, H);
        const bw = s.x1 - s.x0;
        for (let y = 46, k = 0; y < H - 90; y += 84, k++) {
          const grad = g.createLinearGradient(0, y, 0, y + 64);
          const pair = [["#8e2432", "#c04454"], ["#274b8e", "#4a74c2"], ["#8a6f2a", "#c9a83f"]][k % 3];
          grad.addColorStop(0, pair[1]); grad.addColorStop(1, pair[0]);
          g.fillStyle = grad;
          g.beginPath();
          g.moveTo(s.x0 + bw / 2 - 22, y); g.lineTo(s.x0 + bw / 2 + 22, y);
          g.lineTo(s.x0 + bw / 2 + 22, y + 48); g.quadraticCurveTo(s.x0 + bw / 2, y + 72, s.x0 + bw / 2 - 22, y + 48);
          g.closePath(); g.fill();
          g.strokeStyle = "rgba(0,0,0,.5)"; g.lineWidth = 2; g.stroke();
          g.fillStyle = "rgba(255,226,164,.85)";
          g.beginPath();
          g.moveTo(s.x0 + bw / 2, y + 10); g.lineTo(s.x0 + bw / 2 + 9, y + 21); g.lineTo(s.x0 + bw / 2, y + 32); g.lineTo(s.x0 + bw / 2 - 9, y + 21);
          g.closePath(); g.fill();
        }
      }
    }
    ...
    // chalk doodles (school) / glowing rune circles (dungeon) on the backing
    g.strokeStyle = DUNGEON ? "rgba(126,230,210,.08)" : "rgba(255,255,255,.07)"; g.lineWidth = 1.6;
    for (let i = 0; i < 16; i++) {
      const x = ox + hash(i * 11 + 2) * BW, y = oy + hash(i * 13 + 5) * BH;
      g.beginPath();
      if (DUNGEON) { g.arc(x, y, 5 + hash(i) * 5, 0, PI2); }
      else if (i % 3 === 0) { g.arc(x, y, 5 + hash(i) * 4, 0, PI2); }
    ...
    // locker walls (school) / stone brick walls (dungeon)
    for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
      if (!wall[r][c]) continue;
      const x = ox + c * T, y = oy + r * T;
      if (DUNGEON) {
        const jy = (hash(c * 31 + r * 17) - 0.5) * 2.2;
        const wc = st0.wall || {};
        const base = wc.base || "#575f52", mortar = wc.mortar || "#2c322c", top = wc.top || "#74806e";
        g.fillStyle = (c + r) % 3 === 0 ? mortar : base;
        rr(g, x + 1, y + 1, T - 2, T - 2, 2); g.fill();
        g.strokeStyle = mortar; g.lineWidth = 1.2; g.stroke();
        g.fillStyle = top; g.globalAlpha = 0.55;
        g.fillRect(x + 2, y + 2, T - 4, 2.6); g.globalAlpha = 1;
        g.strokeStyle = "rgba(0,0,0,.3)"; g.lineWidth = 1;
        g.beginPath(); g.moveTo(x + 2, y + T * 0.52 + jy); g.lineTo(x + T - 2, y + T * 0.48 + jy); g.stroke();
        if (hash(c * 71 + r * 13) > 0.9 && torches.length < 14) torches.push({ x: x + T / 2, y: y + T / 2 });
        continue;
      }
    ...
    // teachers' lounge (school) / the keep (dungeon): warm wood + iron bars
    ...
    if (door) {
      ...
      g.fillStyle = DUNGEON ? "#241c16" : "#2c3a66"; rr(g, dx + 3, dy + 2, T - 6, T - 3, 2); g.fill();
      g.strokeStyle = DUNGEON ? "#120d09" : "#1a2544"; g.lineWidth = 2; g.stroke();
      if (DUNGEON) {
        g.strokeStyle = "rgba(214,214,224,.55)"; g.lineWidth = 1.6;
        for (let i = 1; i <= 5; i++) {
          const bx = dx + 3 + ((T - 6) * i) / 6;
          g.beginPath(); g.moveTo(bx, dy + 3); g.lineTo(bx, dy + T - 2); g.stroke();
        }
      }
    ...
    // classroom clock (school) / brazier sconce (dungeon) — live flicker drawn separately
    if (!DUNGEON) {
      ...
      g.fillStyle = "#4a3a2c"; rr(g, W / 2 - 13, 26, 26, 12, 3); g.fill();
      g.strokeStyle = "#241a12"; g.lineWidth = 2; g.stroke();
      g.fillStyle = "#4a3a2c"; g.strokeStyle = "#241a12"; g.lineWidth = 2;
      g.beginPath(); g.moveTo(W / 2 - 11, 38); g.lineTo(W / 2 + 11, 38); g.lineTo(W / 2 + 7, 44); g.lineTo(W / 2 - 7, 44); g.closePath();
      g.fillStyle = "#4a3a2c"; g.fill(); g.stroke();
      g.strokeStyle = "#3a2c20"; g.lineWidth = 2;
      g.beginPath(); g.moveTo(W / 2, 26); g.lineTo(W / 2, 16); g.stroke();
    }
  })();
  ...
  function update(dt, input) {
    ...
    for (const g of ghosts) {
      ...
      if (dist2(pa.x, pa.y, ga.x, ga.y) < (T * 0.62) * (T * 0.62)) {
        ...
        else if (G.invuln > 0) continue;
        else if (G.shields > 0) {
          G.shields--; G.invuln = R.invulnSeconds;
          const a2 = anchor(P);
          popup(a2.x, a2.y - 10, "SHIELD BROKE!", "#ffb85c", 15);
          burst(a2.x, a2.y, "#ffd23f", 12, 100);
    ...
    if (G.notebook) {
      ...
      if (nb.ttl <= 0) G.notebook = null;
        ...
        G.score += 500;
        popup(nx, ny - 10, DUNGEON ? "LOST RELIC! +500" : "GOLD STAR! +500", GOLD, 15);
  ...
  // ---- draw -------------------------------------------------------------------
  // Knight (dungeon): armored dome + helm slit, red plume, shield aura, tucked sword
  function drawKnight(ctx) {
    ...
    ctx.beginPath(); ctx.ellipse(a.x, a.y + R1 * 0.85, R1 * 0.8, R1 * 0.28, 0, 0, PI2); ctx.fill();
    if (G.shields > 0) {
      const pulse = 1 + 0.07 * Math.sin(G.t * 5);
      ctx.strokeStyle = "rgba(255,210,63,.8)"; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(a.x, a.y, R1 * 1.22 * pulse, 0, PI2); ctx.stroke();
      const ha = ctx.createRadialGradient(a.x, a.y, R1 * 0.8, a.x, a.y, R1 * 1.5 * pulse);
      ha.addColorStop(0, "rgba(255,210,63,.28)"); ha.addColorStop(1, "rgba(255,210,63,0)");
      ctx.fillStyle = ha; ctx.beginPath(); ctx.arc(a.x, a.y, R1 * 1.5 * pulse, 0, PI2); ctx.fill();
    }
    ...
    const grd = ctx.createRadialGradient(-R1 * 0.2, -R1 * 0.3, R1 * 0.2, 0, 0, R1);
    grd.addColorStop(0, "#eef2f8"); grd.addColorStop(0.6, "#c7cfdd"); grd.addColorStop(1, "#8b95a8");
    ctx.fillStyle = grd; ctx.strokeStyle = INK; ctx.lineWidth = 2.6; ctx.lineJoin = "round";
    ctx.beginPath(); ctx.moveTo(0, 0);
    ctx.arc(0, 0, R1, m, PI2 - m);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#2c2a55";
    ctx.beginPath(); ctx.ellipse(R1 * 0.42, -R1 * 0.28, R1 * 0.17, R1 * 0.12, 0, 0, PI2); ctx.fill();
    ctx.restore();
    // plume stays upright
    ctx.save();
    ctx.translate(a.x, a.y - R1 * 1.05); ctx.scale(scale, scale);
    ctx.fillStyle = "#c23a2a"; ctx.strokeStyle = INK; ctx.lineWidth = 1.8;
    ctx.beginPath(); ctx.moveTo(-R1 * 0.2, 0);
    ctx.quadraticCurveTo(-R1 * 0.46, -R1 * 0.52 + Math.sin(G.t * 2.6) * 2, 0, -R1 * 0.62);
    ctx.quadraticCurveTo(R1 * 0.4, -R1 * 0.4, R1 * 0.16, 0);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.restore();
    // sword tucked at the side
    ctx.save();
    ctx.translate(a.x, a.y); ctx.rotate(0.5 + (face.x < 0 ? Math.PI : 0));
    ctx.strokeStyle = "#aab4c6"; ctx.lineWidth = 3; ctx.lineCap = "round";
    ctx.beginPath(); ctx.moveTo(R1 * 0.7, R1 * 0.55); ctx.lineTo(R1 * 1.15, R1 * 0.1); ctx.stroke();
    ctx.strokeStyle = "#8a6a3a"; ctx.lineWidth = 2.4;
    ctx.beginPath(); ctx.moveTo(R1 * 0.62, R1 * 0.63); ctx.lineTo(R1 * 0.78, R1 * 0.47); ctx.stroke();
    ctx.restore();
    ctx.globalAlpha = 1;
  }
  ...
  function drawGhost(ctx, g) {
    ...
    } else spr = (g.wyrm ? WYRM_SPRITES : GHOST_SPRITES)[g.wyrm ? 0 : g.idx][fr];
    blit(ctx, spr, x, y, T * 1.1, { flip: g.dir && g.dir.x < 0 ? -1 : 1 });
  }

  function draw(ctx) {
    ctx.drawImage(BG, 0, 0);
    // torch flames flicker live (dungeon)
    if (DUNGEON) for (let i = 0; i < torches.length; i++) {
      const t = torches[i];
      const halo = ctx.createRadialGradient(t.x, t.y - 8, 2, t.x, t.y - 8, 26);
      halo.addColorStop(0, "rgba(255,168,74,.3)"); halo.addColorStop(1, "rgba(255,168,74,0)");
      ctx.fillStyle = halo; ctx.beginPath(); ctx.arc(t.x, t.y - 8, 26, 0, PI2); ctx.fill();
      ctx.save();
      ctx.translate(t.x, t.y - 9 + Math.sin(G.t * 11 + i * 2.1) * 1.2);
      ctx.scale(1 + Math.sin(G.t * 9 + i * 1.7) * 0.18, 1);
      ctx.fillStyle = "#ffb85c";
      ctx.beginPath(); ctx.moveTo(0, -7); ctx.quadraticCurveTo(4.4, -1, 0, 5); ctx.quadraticCurveTo(-4.4, -1, 0, -7); ctx.fill();
      ctx.fillStyle = "#ffe98d";
      ctx.beginPath(); ctx.ellipse(0, 1, 1.8, 3, 0, 0, PI2); ctx.fill();
      ctx.restore();
    }
    ctx.save();
    ctx.translate(ox, oy);
    ...
    // facts + apples (runes in the dungeon)
    for (const k in dots) {
      const d = dots[k];
      if (d.got) continue;
      ...
      if (d.apple) {
        ...
        halo.addColorStop(0, DUNGEON ? "rgba(126,230,210,.45)" : "rgba(255,90,78,.4)");
        halo.addColorStop(1, DUNGEON ? "rgba(126,230,210,0)" : "rgba(255,90,78,0)");
        ctx.fillStyle = halo;
        ctx.beginPath(); ctx.arc(x, y, T * 0.9, 0, PI2); ctx.fill();
        ctx.translate(x, y); ctx.scale(pulse, pulse);
        if (DUNGEON) ctx.drawImage(RUNE, -22, -22);
        else ctx.drawImage(APPLE, -22, -22);
        ctx.restore();
      } else if (DUNGEON) {
        const tierCols = ["#b06a2e", "#d5dbe4", "#ffd23f"];
        ctx.fillStyle = tierCols[d.tier || 0];
        ctx.beginPath(); ctx.arc(x, y, d.tier ? 4.4 : 3.2, 0, PI2); ctx.fill();
        if (d.tier) { ctx.strokeStyle = "rgba(36,31,61,.55)"; ctx.lineWidth = 1; ctx.stroke(); }
      } else {
        ctx.fillStyle = CREAM;
    ...
    if (G.notebook) {
      ...
      ctx.drawImage(DUNGEON ? SCROLL : NOTEBOOK, -26, -26);
      ctx.restore();
    }
    ...
    if (G.status !== "bell" || Math.floor(G.t * 8) % 2 === 0) (DUNGEON ? drawKnight : drawPlayer)(ctx);
    // popups + particles
    ...
    if (G.status === "ready") {
      ...
      drawText(ctx, DUNGEON ? "THE DUNGEON FALLS!" : "THE BELL RANG!", cxm, cym - 12, { size: 32, color: GOLD, shadow: "rgba(0,0,0,.6)" });
      drawText(ctx, DUNGEON ? "GLORY · EVERY COIN LOOTED" : "A+ · EVERY FACT MUNCHED", cxm, cym + 16, { size: 14, color: "#fdf2d0" });
    }
    if (G.fright > 0 && G.status === "playing") {
      const blink = G.fright < 2 && Math.floor(G.t * 6) % 2 === 0;
      if (!blink) drawText(ctx, TERROR_WORD, cxm, oy - 22, { size: 20, color: DUNGEON ? "#7de2c8" : "#a8ecf2", shadow: "rgba(0,0,0,.55)" });
    }
    // live clock hands (school) / live brazier flame (dungeon)
    if (!DUNGEON) {
      ...
      const fl = 1 + Math.sin(G.t * 8) * 0.2;
      ctx.save();
      ctx.translate(W / 2, 24); ctx.scale(fl, fl);
      ctx.fillStyle = "#ffb85c";
      ctx.beginPath(); ctx.moveTo(0, -6); ctx.quadraticCurveTo(4, 0, 0, 5); ctx.quadraticCurveTo(-4, -1, 0, -6); ctx.fill();
      ctx.restore();
    }
    if (DBG) {
  ...
  if (DBG || AP) {
    window.__TEST_API__ = {
      describe: () => "maze genre · Study Hall Scramble · autopilot playtest hooks",
      snapshot: () => ({
        ...
        ghosts: ghosts.map((g) => ({
          who: g.spec.key, phase: g.phase, fright: g.fright, eaten: g.eaten, wyrm: !!g.wyrm,
      ...
      debug: () => ({
        ...
        grant: (n) => { addXp(n || 0); return { xp: G.xp, lvl: G.xpLvl, shields: G.shields }; },
        makeWyrm: () => { const g = ghosts[0]; if (!g) return false; g.wyrm = true; return true; },
        setShields: (n) => { G.shields = Math.max(0, n | 0); return G.shields; },
      }),
    };
